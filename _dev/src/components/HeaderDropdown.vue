...
<template>
  <Popover v-slot="{ open, close }" class="relative">
      <label :for="name" v-if="label" class="block text-sm font-medium text-gray-700"
      >{{ label }}</label>
    <PopoverButton
    ...
    >
      <PopoverPanel
        class="absolute left-1/2 z-10 mt-3 w-80 -translate-x-1/2 transform px-4 sm:px-0 lg:max-w-3xl"
      >
        <div
