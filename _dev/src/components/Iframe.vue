<template>
    ...
    <section class="w-full h-full">
        ...
        <!-- {{ iframe.loader }} -->
        <Loader :visible="iframe.loader.value"  class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">Chargement en cours</Loader>
        {{ classes }}
        <iframe  id="website-iframe" sandbox="allow-modals allow-forms allow-popups allow-scripts allow-same-origin" 
        :class="[height, width]"
        class="border-none h-full mx-auto rounded" :src="iframe.current_url.value" frameborder="0"></iframe>
